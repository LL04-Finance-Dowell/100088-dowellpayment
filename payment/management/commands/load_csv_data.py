...
# Import the model 
from payment.models import (WorkFlowAI,WifiQrcode,DigitalQ,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for row in DictReader(open('./Costing for payment api  - WIFI QR CODE.csv')):
            wifi_qrcode=WifiQrcode(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
            wifi_qrcode.save()

        for row in DictReader(open('./Costing for payment api  - DIGITAL Q.csv')):
            digital_q=DigitalQ(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
            digital_q.save()

        for row in DictReader(open('./Costing for payment api  - LOGO SCAN .csv')):
            logo_can=LogoScan(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
            logo_can.save()


        for row in DictReader(open('./Costing for payment api  - NPS.csv')):
            nps=Nps(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
            nps.save()


        for row in DictReader(open('./Costing for payment api  - VOC.csv')):
            voc=Voc(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
            voc.save()


        for row in DictReader(open('./Costing for payment api  - UX LIVE.csv')):
            ux_live=UxLive(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
            ux_live.save()


        for row in DictReader(open('./Costing for payment api  - SOCIAL MEDIA AUTOMATION .csv')):
            social_media_automation=SocialMediaAutomation(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
            social_media_automation.save()

        for row in DictReader(open('./Costing for payment api  - LICENSE COMPATIBILITY.csv')):
            license_compatibility=LicenseCompatibility(country_name=row['Country Name'], currency_name=row['CURRENCY NAME'],
                                   currency_code=row['CURRENCY CODE'],price=row['PRICE'])  
