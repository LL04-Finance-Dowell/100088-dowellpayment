    ...
    def post(self, request, api_key):
        ...
        try:
            ...
            if payment.create():
                ...
                return Response(
                    {"approval_url": approval_url}, status=status.HTTP_200_OK
                )
            else:
        ...
        try:
            ...
            try:
                ...
                session = stripe.checkout.Session.create(
                    ...
                    payment_intent_data={
                        "metadata": {"description": f"{product}", "date": today}
                    },
                )
                return Response(
                    {"approval_url": f"{session.url}"}, status=status.HTTP_200_OK
                )
...
class PaypalPaymentLink(APIView):
    @swagger_auto_schema(
        request_body=PaypalPaymentLinkSerializer, responses={200: "checkout url"}
    )
    def post(self, request, api_key):
        validate = processApikey(api_key)
        try:
...
class StripePaymentLink(APIView):
    @swagger_auto_schema(
        request_body=StripePaymentLinkSerializer, responses={200: "checkout url"}
    )
    def post(self, request, api_key):
        validate = processApikey(api_key)
        try:
